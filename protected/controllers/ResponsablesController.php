...
class ResponsablesController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Responsables']))
		{
			...
			if($model->save())
				$this->redirect(array('view','id'=>$model->keyR));
		}
	...
	public function loadModel($id)
	{
        ...
        public function cargarDependientes($criteria)
	{
             //$Criteria = new CDbCriteria();
              //$Criteria->condition = "keyR=1";
              $criteria=New Dependientes();
              
              $criteria->keyR=1;
              $modelDependientes=Dependientes::model()->findAllBySql(
                      'select * from dependientes
       ');
                      

            //Model::model()->findAllByAttributes(array('id'=>1),array('order'=>'id DESC'));
		//$modelDependientes=Dependientes::model()->findAllByAttributes(array('keyR'=>2),$criteria);
		if($modelDependientes===null)
			throw new CHttpException(404,'The requested page does not exist.');
		return $modelDependientes;
	}
       ...
       public function actionFancybox()
{
    if(Yii::app()->request->isAjaxRequest)
    {
       ...
       public function abrirVentana($link)
	{
		...
		{   
			$model->attributes=$_POST['Dependientes'];
			if($model->save()){
				$this->redirect(array('view','id'=>$model->keyD));
                        }
		}
